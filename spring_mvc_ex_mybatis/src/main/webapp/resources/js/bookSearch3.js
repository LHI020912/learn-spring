$(document).ready(function(){
 	$("#bookSearchFrm3").on('submit', function(e) {
    	e.preventDefault();
 		...
 		if(keyword==""||type==""){ alert("검색조건과 검색어를 입력하세요"); }
 		else{
 			$.ajax({
 			  type:"post",
 			  url: "/mybatis/book/bookSearch3",
 			  data:formData,
 			  success:function(result){
 			  		console.log(result);
 			  		$("#searchResultBox").empty();
 			  		$("#searchResultBox").append('<table id="resultTable" border="1" width="500">'+
  												 '<tr><th>도서번호</th><th>제목</th><th>작가</th>' +
	 											 '<th>도서가격</th><th>출고일</th><th>재고</th><th>출판사</th></tr>');
					 if(result==""){
