...
public class fileController {
	...
	// 업로드 되는 파일 크기 : 기본값이 maxFileSize:1M(개별파일크기), maxRequestSize:10MB(한번에 전송되는 모든 파일 크기의 합)
	@PostMapping("/fileUpload")
	public String fileUpload(@RequestParam("uploadFile") MultipartFile file, Model model) throws IOException {
		// 파일이 서버로 전송될 때 서버는 : 해당파일 이름 + UUID
		String uploadPath="D:/springbootWorkspace/upload/";
		
		// 원본파일 이름 저장
		String originalFileName = file.getOriginalFilename(); // 전송될 파일의 원본명
	...
	public String fileOriginalNameUpload(@RequestParam("uploadFileOriginal") MultipartFile file,
										 Model model) throws IOException {
		// 파일이 서버로 전송될 때 서버는 : 해당파일 이름 + UUID
		String uploadPath="D:/springbootWorkspace/upload/product_image/";
